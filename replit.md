...
- **✓ Franchise Filtering Bug Fix**: Fixed critical bug where users with role "admin" and franchise association were seeing all franchises instead of only their associated franchise. Updated filtering logic to properly distinguish between global admins (admin role without franchise association) and franchise-level admins (admin role with franchise association)
...
- **✓ Role-Based Match Creation Access Control**: Restricted match creation to only admin and global_admin roles with proper UI hiding and access validation. Removed "Create Match" from navigation menu, now only accessible via button on matches page for authorized users
